...
namespace Seo
{
    public class Package
    {
        ...
        public static string ExtensionDescription = Seo.Language.Package.Extention;
        // 包ID
        ...
        public static void Create(string path, string name, string creator, string description, string image, bool isTemp)
        {
            ...
            desFile.WriteString(BasicSection, "Version", Seo.Language.Application.Version);
        ...
        private static Package Open(string packagePath)
        {
            ...
            {
                return new Package(false, Seo.Language.Package.ImportInvalidPackage);
            }
            // 检查Id是否合法
            ...
            if (Regex.IsMatch(id, @"\b[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]-[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]-[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]-[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]\b"))
            {
                ...
                package.Name = info.ReadString(BasicSection, "Name-" + Seo.Language.LanguageManager.LocalLanguage, package.Name);
                ...
                package.Creator = info.ReadString(BasicSection, "Creator-" + Seo.Language.LanguageManager.LocalLanguage, package.Creator);
                package.Description = info.ReadString(BasicSection, "Description", String.Empty);
                package.Description = info.ReadString(BasicSection, "Description-" + Seo.Language.LanguageManager.LocalLanguage, package.Description);
                ...
                {
                    return new Package(false, Seo.Language.Package.ImportExistedPackage);
                }
                return package;
            }
            else
            {
                return new Package(false, Seo.Language.Package.ImportUnknowPackage);
            }
        }
